...
class BiddSportWidget {
  ...
  async updateBadge() {
    ...
    try {
      // Check if tournaments data exists (may not be loaded yet)
      if (typeof tournaments === 'undefined' || !Array.isArray(tournaments)) {
        console.log('⏳ Tournaments data not ready yet');
        return;
      }
  ...
  getWidgetData() {
    return {
      activeTournaments: (typeof tournaments !== 'undefined' && Array.isArray(tournaments)) ? tournaments.filter(t => new Date(t.date) >= new Date()).length : 0,
      totalEvents: (typeof events !== 'undefined' && Array.isArray(events)) ? events.length : 0,
      totalCompetitors: (typeof competitors !== 'undefined' && Array.isArray(competitors)) ? competitors.length : 0,
      recentResults: (typeof results !== 'undefined' && Array.isArray(results)) ? results.slice(-5) : [],
