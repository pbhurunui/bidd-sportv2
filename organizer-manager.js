...
async function syncOrganizerToFirestore(organizerEmail, roleTemplate, permissions) {
    ...
    if (typeof firebase === 'undefined' || !firebase.firestore || !idToken || !currentUser) {
        console.warn('⚠️ Firebase SDK not available or user not logged in - skipping Firestore sync');
        return false;
    }
...
async function removeOrganizerFromFirestore(organizerEmail) {
    if (typeof firebase === 'undefined' || !firebase.firestore) {
        console.warn('⚠️ Firebase SDK not available - skipping Firestore removal');
        return false;
    }
    try {
        const db = firebase.firestore();
